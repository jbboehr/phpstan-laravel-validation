...
| Numeric checks | `Decimal`, `Digits`, `DigitsBetween`, `Integer`, `MaxDigits`, `MinDigits`, `MultipleOf`, `Numeric` | Version-aware unions of the native values Laravel accepts and preserves; see [Numbers](validation-rules.md#numbers) for coercive predicates and their broader families |
